...
#include "tinymt32def.h"
    ...
    // Returns a buffer of tinyMT params taken from filename.

    cl::Buffer get_params_buffer(std::string filename, cl::Context &context, cl::CommandQueue &queue, int total_num)
    {
    	...
    	for (int i = 0; i < total_num; i++) {
    		...
    		status_tbl[i].mat1 = mat1; status_tbl[i].mat2 = mat2; status_tbl[i].tmat = tmat;
    	}
    	cl::Buffer status_buffer(context, CL_MEM_READ_ONLY, total_num * sizeof(tinymt32wp_t));

    	queue.enqueueWriteBuffer(status_buffer, CL_TRUE, 0, total_num * sizeof(tinymt32wp_t), status_tbl);
    	delete[] status_tbl;
